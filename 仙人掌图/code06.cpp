...
int fromWeight[MAXN]; // 来到i的边的边权
int cycleLen[MAXN];   // 环顶节点到点i的路径累加和为cycleLen[i]
int cycleSum[MAXN];   // 环上路径的累加和
...
void dfs1(int u, int f, int l)
{
    ...
    for (int e = head2[u], v; e > 0; e = next2[e])
    {
        v = to2[e];
        if (v != f)
        { // 没用的，我这次圆方树是建的单向边，回不去的，只是模板是这么写的，我就写上了
