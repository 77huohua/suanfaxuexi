...
void maintain(int i) {
	xmin[i] = min({ x[i],xmin[ls[i]],xmin[rs[i]] });
	xmax[i] = max({ x[i],xmax[ls[i]],xmax[rs[i]] });
	ymin[i] = min({ y[i],ymin[ls[i]],ymin[rs[i]] });
	ymax[i] = max({ y[i],ymax[ls[i]],ymax[rs[i]] });
}
...
int main() {
	...
	xmin[0] = ymin[0] = LLONG_MAX;
	xmax[0] = ymax[0] = LLONG_MIN;
	root = build1(1, n, 0);
	ans = dist(1, 2);
	for (int i = 1;i <= n;i++) {
		updateAns(i, root);
	...
	cout << format("{:.4f}", sqrt((double)ans)) << endl;
}